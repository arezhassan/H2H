...
public class RiderConsignmentAdapter extends RecyclerView.Adapter<RiderConsignmentAdapter.ConsignmentViewHolder> {
    ...
    public RiderConsignmentAdapter(List<Consignment> consignmentList, Context context, List<Rider> riders) {
    ...
    public ConsignmentViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        // Inflate the default layout for other conditions

        itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.singleconsignmentrider, parent, false);
    ...
    public void onBindViewHolder(@NonNull ConsignmentViewHolder holder, int position) {
        ...
        if (consignment.getRider()!=null && !consignment.getRider().isEmpty()) {
            holder.btnShowLocation.setVisibility(View.GONE);
        } else {
            holder.btnShowLocation.setVisibility(View.VISIBLE);
        }


        // Now you can use 'itemView' to access views in the inflated layout
        ...
        if (consignment.getWeight()!=null && !consignment.getWeight().isEmpty()) {
           holder.textinputweight.getEditText().setText(consignment.getWeight());
        }
        if (consignment.getPrice()!=null && !consignment.getPrice().isEmpty()) {
        ...
        holder.btnChangeStatus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!consignment.getRider().isEmpty()) {
                    ...
                    holder.btnCalculateShipping.setVisibility(View.GONE);
                    holder.btnConfirmDetails.setText("Set Status");
                } else {
        ...
        holder.btnConfirmDetails.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (consignment.getRider().isEmpty()) {
                    ...
                    if (status!=null && status.equals("Select Consignment Status")) {
                        Toast.makeText(context, "Please select consignment status", Toast.LENGTH_SHORT).show();
                    } else {
                        status = holder.spinnerStatus.getSelectedItem().toString();
                        String userid = consignment.getUserid();
